...
public class ShardedJedisSentinelPool extends Pool<ShardedJedis> {
	...
	private void initPool(List<HostAndPort> masters) {
		if (!equals(currentHostMasters, masters)) {
			...
			initPool(poolConfig,
					new ShardedJedisFactory(shardMasters, Hashing.MURMUR_HASH, Sharded.DEFAULT_KEY_TAG_PATTERN));
					...
					try {
						...
						if (master == null) {
							...
							if (hostAndPort != null && hostAndPort.size() > 0) {
								if (hostAndPort.get(0).equals("127.0.0.1")
										|| hostAndPort.get(0).equalsIgnoreCase("localhost")) {
									hostAndPort.set(0, hap.getHost());
								}
					...
					jedis.subscribe(new JedisPubSubAdapter() {
						...
						public void onMessage(String channel, String message) {
							...
							if (switchMasterMsg.length > 3) {
								...
								if (index >= 0) {
									String tempHost = switchMasterMsg[3];
									if (tempHost.equals("127.0.0.1") || tempHost.equalsIgnoreCase("localhost")) {
										tempHost = host;
									}
									HostAndPort newHostMaster = toHostAndPort(
											Arrays.asList(tempHost, switchMasterMsg[4]));
